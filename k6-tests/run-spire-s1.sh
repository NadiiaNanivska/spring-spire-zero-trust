...
RESULTS_DIR="test-results-spire-s1-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$RESULTS_DIR"
...
# Функція експорту (з твого файлу)
...
echo -e "${BLUE}🚀 Starting single test: SPIRE Scenario 1${NC}"
echo -e "Target: ${GREEN}$SPIRE_URL${NC}\n"

START_TIME=$(date -u +%s)

# Виклик тільки для SPIRE
